...
int ledPin = 12;      // LED 꽂혀있는 핀번호
// 변수 변경:
int delaybase = 25;
//100 펜리더는 100이 가장 좋은것 같은데?
//20 은 코드 39랑 레이저 리더에 좋음.

#define STR_LENGTH 50 //500 should give us 100 Code 128 characters and 50 Code 39
...
void loop()
{
  ...
  if (sr == "6"){ //간단한 바코드 SQL 인젝션공격
    SendUSingDIPChoice("' or 1=1 -- ");
  }
  if (sr == "7"){//백신이 반응을 보이는지 테스트할 EICAR 문자열 
    SendUSingDIPChoice("X5O!P%@AP[4\\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*");
  }
  if (sr == "8"){
    Code128StringSend("TRY TO PASTE v",103); //v 는 128a 에서 Ctrl+V 코드임.
  }
  if (sr == "9"){//몇몇 오래된 장비에 보내서 어떤 키입력으로 들어가는지 판단
    int points[]={
      64,65,66,67,68,69,70,71,72,73,74,75,75,77,78,79,80,81,82,83,84,85,86,87,88,89,90,91,92,93,94,95        };
    Code128IntArrSend(points,  103,  31);
...
*/
  ...
  char buf[6];
  void SendUSingDIPChoice(char *TestString) {
    Code128StringSend(strcat(TestString, itoa(delaybase, buf, 104)));
  }
  delaybase++;
  if (delaybase>1000){
    delaybase=1;
